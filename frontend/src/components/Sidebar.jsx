...
import { 
  ...
  Pill, ClipboardList, Building2, History, Activity,
  BedDouble, Receipt, Timer, BarChart,
  Shield, Layout, Menu, Sun, Moon, LogOut, PillBottle, Package, Boxes, Tablets
} from 'lucide-react';
...
const DEFAULT_MENUS = [
  ...
  },
  {
    ...
    icon: 'master',
    children: [
      { id: 21, menu_name: 'Pasien', menu_path: '/master/pasien', icon: 'pasien' },
      { id: 22, menu_name: 'Dokter', menu_path: '/master/dokter', icon: 'dokter' },
      { id: 23, menu_name: 'Obat', menu_path: '/master/obat', icon: 'obat' },
      { id: 24, menu_name: 'Tindakan', menu_path: '/master/tindakan', icon: 'tindakan' },
  ...
  },
  {
    ...
    children: [
      ...
      { id: 32, menu_name: 'Riwayat RM', menu_path: '/rm/history', icon: 'history' },
      { id: 33, menu_name: 'Diagnosa', menu_path: '/rm/diagnosa', icon: 'diagnosa' }
    ]
  },
  {
    id: 4,
    ...
    icon: 'pendaftaran',
    children: [
      { id: 41, menu_name: 'Rawat Jalan', menu_path: '/pendaftaran/rawat-jalan', icon: 'rawat' },
      { id: 42, menu_name: 'Rawat Inap', menu_path: '/pendaftaran/rawat-inap', icon: 'rawat' }
    ]
  },
  {
    id: 5,
    ...
    icon: 'kasir',
    children: [
      { id: 51, menu_name: 'Resep Obat', menu_path: '/kasir/resep', icon: 'file-medical' },
      { id: 51, menu_name: 'Pembayaran', menu_path: '/kasir/pembayaran', icon: 'kasir' },
      { id: 52, menu_name: 'Invoice', menu_path: '/kasir/invoice', icon: 'invoice' },
      { id: 53, menu_name: 'Tagihan Pasien', menu_path: '/kasir/tagihan', icon: 'tagihan' }
    ]
  },
  {
    id: 6,
    ...
    icon: 'antrian',
    children: [
      { id: 61, menu_name: 'Antrian Poli', menu_path: '/antrian/poli', icon: 'antrian' },
      { id: 62, menu_name: 'Antrian Kasir', menu_path: '/antrian/kasir', icon: 'antrian' }
    ]
  },
  {
    id: 7,
    ...
    icon: 'laporan',
    children: [
      { id: 71, menu_name: 'Kunjungan', menu_path: '/laporan/kunjungan', icon: 'kunjungan' },
      { id: 72, menu_name: 'Diagnosa', menu_path: '/laporan/diagnosa', icon: 'diagnosa' },
      { id: 73, menu_name: 'Pendapatan', menu_path: '/laporan/pendapatan', icon: 'laporan' }
    ]
  },
  {
    id: 8,
    ...
    children: [
      ...
      { id: 82, menu_name: 'Role & Privilege', menu_path: '/settings/role', icon: 'shield' },
      { id: 83, menu_name: 'Master Menu', menu_path: '/settings/menu', icon: 'menu' }
    ]
  }
];
    ...
    <div ref={sidebarRef} className="fixed left-0 h-screen w-64 bg-gray-50 border-r border-gray-200 text-gray-700 flex flex-col z-[9]">
      ...
      <div className="flex items-center gap-2 p-4 bg-white border-b border-gray-200">
        ...
        <svg width="150" height="32" viewBox="0 0 486 123" fill="none" xmlns="http://www.w3.org/2000/svg">
          ...
          <path fillRule="evenodd" clipRule="evenodd" d="M463.983 25.7713L461.028 20.6524L466.023 12.1482L458.871 0.425009L454.1 8.65276L449.104 0L440.995 14.0459L454.553 37.5299L470.772 37.5299L470.759 37.5064L481.068 37.5249L486 25.7713L463.983 25.7713Z" fill="#095D7E"/>
      ...
      <nav className="flex-1 py-4 overflow-y-auto">
        {menus.map((menu) => {
          ...
          const isActiveParent = hasChildren && menu.children.some(child => location.pathname === child.menu_path);
              ...
              {activeMenu === menu.id && hasChildren && (
                <div className="mt-1 pl-9">
                  {menu.children.map(child => {
                    const ChildIcon = ICON_MAP[child.icon?.toLowerCase()] || Icon;
